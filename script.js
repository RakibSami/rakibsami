...
function initializeCanvas() {
        ...
        for (let i = 0; i < particleCount; i++) {
            canvasParticles.push({
                ...
                color: currentTheme === 'dark' ? 
                    ...
                    `rgba(255, 245, 200,${Math.random() * 0.3})` // soft pastel yellow
            });
        }
    }
    ...
    function animateParticles() {
        ...
        // Gradient background inspired by "Take Me Home, Country Roads"
        const gradient = canvasCtx.createLinearGradient(0, 0, canvas.width, canvas.height);
        if (currentTheme === 'dark') {
            gradient.addColorStop(0, '#1a2030'); // soft midnight blue
            gradient.addColorStop(1, '#3b3f58'); // muted purple
        } else {
            gradient.addColorStop(0, '#a8edea'); // soft sky blue
            gradient.addColorStop(1, '#fed6e3'); // pastel peach-pink
        }
            ...
            canvasParticles.forEach(other => {
                ...
                if (distance < 100) {
                    ...
                    canvasCtx.strokeStyle = currentTheme === 'dark' ? 
                        `rgba(180, 200, 255,${0.2 * (1 - distance/100)})` : 
                        `rgba(255, 245, 200,${0.2 * (1 - distance/100)})`;
...
function updateCanvasTheme(theme) {
    ...
    canvasParticles.forEach(p => {
        p.color = theme === 'dark' ? 
            `rgba(180, 200, 255,${Math.random() * 0.3})` : 
            `rgba(255, 245, 200,${Math.random() * 0.3})`;
    });
}
...
function showNotification(msg,type){
    ...
    notification.style.cssText=`position:fixed;top:100px;right:20px;background:${type==='success'?'#4CAF50':'#f44336'};color:white;padding:15px 20px;border-radius:8px;box-shadow:0 5px 15px rgba(0,0,0,0.2);display:flex;align-items:center;gap:10px;z-index:9999;transform:translateX(100%);transition:transform 0.3s ease;`;
